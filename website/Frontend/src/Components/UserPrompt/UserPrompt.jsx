...
export default function UserPrompt({ className, children, showIcon = false, value, onChange, ...props }) {
    return (
        <div className={twMerge("flex font-poppins text-tan text-[.75rem] md:text-[1rem] pl-4 pr-2 py-1.5 border-red-orange border-1 rounded-4xl w-full items-center cursor-text", className)}>
            <textarea
                value={value}
                onChange={onChange}
                className="flex-grow h-full w-full bg-transparent outline-none resize-none overflow-hidden leading-normal p-0 text-start"
                placeholder={children}  // Use children as the placeholder
                rows={1} // Adjust this based on the required height
            />
            {showIcon && (
